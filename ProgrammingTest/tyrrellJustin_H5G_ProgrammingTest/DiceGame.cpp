...
namespace DiceGame {
	...
	int Game::RunDiceGame() {
		//instance->player->PopulatePlayerDiceBag(instance->diceFactory);
		
		//GameLoop();
		//AutomatedGameLoop(1000);
	...
	void Game::GameEnd(GameResult* result) {
		...
		//instance->gameTable->ReturnDiceToBag(instance->player->GetMyDiceBag());

		//while (instance->gameTable->GetRemainingNumberOfDiceOnTable() > 0) {
		//
		//	instance->player->ReturnDieToBag(instance->gameTable->RemoveDieFromTable());


		//	//Die* die = instance->gameTable->RemoveDieFromTable();
		//	//instance->player->ReturnDieToBag(die);
		//}

		//result->numDie = instance->player->GetRemainingNumberOfDieInBag();
		//result->numTable = instance->gameTable->GetRemainingNumberOfDiceOnTable();
		instance->AddGameResult(result);
		instance->gameTable->PurgeTableOfDice();
	}

	void Game::RollForGame(GameResult* resultTracking) {
		Die* die = instance->player->RollDie();
	...
	int Game::DisplayMenuOptoins() {
		// Run for 10 itterations
		// Run for 1000 itterations
		// Run for 10,000 itterations
		// Run for 1,000,000 itterations
		// Run for 10,000,000 itterations
		// Run for Custom Amount of itterations (10,000,000 MAX)
		// Run 1 at a time
		int iOption = int();
		do {
			char option = char();
	...
	void Game::ExecuteMenuOption(int option) {
		switch (option) {
			case (1) :
				instance->AutomatedGameLoop(MENU_OPTION_ONE_ITERATIONS);
				break;
			case (2) :
				instance->AutomatedGameLoop(MENU_OPTION_TWO_ITERATIONS);
				break;
			case (3) :
				instance->AutomatedGameLoop(MENU_OPTION_THREE_ITERATIONS);
				break;
			case (4) :
				instance->AutomatedGameLoop(MENU_OPTION_FOUR_ITERATIONS);
				break;
			case (5) :
				instance->AutomatedGameLoop(MENU_OPTION_FIVE_ITERATIONS);
				break;
			case (6) :
				//instance->AutomatedGameLoop(10);
				instance->ExecuteCustomCount();
				break;
			case (7) :
				instance->GameLoop();
				break;
	...
	void Game::ExecuteCustomCount() {
		...
		//cout << num << endl;
		instance->AutomatedGameLoop(num);
		//cin.getline(amount);
		//cin.getline()
	}
	...
	void GameResult::AddDieResult(int result) {
		for (int i = 0; i < BAG_MAX_DICE; ++i) {
			if (this->dieRolls[i] != 0) continue;
				...
				this->dieRolls[i] = result;
				*this->scoreTracker += result;
				break;
			}
		}
	}
